...
def organize_webp_folders(main_folder_path):
    """
    Main function to organize folders from __WEBP To be move to the right folders
    """
...
def main():
    ...
    """
    print("WEBP Folder Organizer")
    print("=" * 50)
    
    # Get main folder path from user
    while True:
        main_folder_path = input("Enter the path to your main folder: ").strip()
    ...
    print(f"\nMain folder: {main_folder_path}")
    print(f"Source folder: {os.path.join(main_folder_path, '__WEBP To be move to the right folders')}")
    
    confirm = input("\nProceed with organization? (y/n): ").strip().lower()
    
    if confirm in ['y', 'yes']:
        organize_webp_folders(main_folder_path)
    else:
