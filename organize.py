...
def organize_webp_folder_only(webp_folder_path):
    """Organize files within the WEBP folder only, creating brand/product/category structure with basename grouping"""
    print(f"Organizing files in WEBP folder: {webp_folder_path}")
    
    # First pass: collect all files and group them by brand and basename
    ...
    # Second pass: group files by basename within each brand
    for brand_name, files in brand_files.items():
        print(f"\nProcessing brand: {brand_name}")
        print(f"Files in brand: {files}")
        
        # Group files by basename
        basename_groups = group_files_by_basename(files)
        
        print(f"Basename groups for {brand_name}: {dict(basename_groups)}")
        
        # Process each basename group
        for basename, files_in_group in basename_groups.items():
            print(f"\nProcessing basename group: {basename}")
            print(f"Files in group: {files_in_group}")
            
            # Determine the product code folder name for this basename group
            product_code = determine_product_code_from_basename_group(files_in_group, brand_name)
            
            print(f"Determined product code: {product_code}")
            ...
            # Process each file in the basename group
...
def organize_webp_files(webp_folder_path):
    """Main function to organize files within the WEBP folder"""
    ...
    print("Starting organization within WEBP folder with basename grouping...")
    
    # Organize files within the WEBP folder
    organize_webp_folder_only(webp_folder_path)
    
    # Remove empty folders within WEBP folder
    remove_empty_folders(webp_folder_path)
